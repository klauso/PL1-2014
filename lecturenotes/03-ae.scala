...
/* Let us now consider an extension of the arithmetic expression
 * language with variables. We do this by a new kind of expression,
 * which we call Identifier, or Id. */
...
// Let's now try the same with the AE language with identifiers. 
...
object AEIdVisitor {
  ...
  // However, what about the evaluator? If we instantiate T=Int, then 
  // how can we access the environment?
  // Insight: For evaluation, we must instantiate T with a function 
  // type Env => Int!
  // This way of transforming a multi-argument function into a
  // single-argument higher-order function is called _currying_.
